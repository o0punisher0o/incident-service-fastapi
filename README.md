# Incident Service with Telegram Integration

Маленький API-сервис для учёта инцидентов с уведомлениями в Telegram.

---

## ⚙️ Технологии
- Python 3.10+
- FastAPI
- SQLite
- SQLAlchemy
- aiohttp
- python-dotenv

---

## 🚀 Установка и запуск

```bash
git clone <repo_url>
cd incident_service
python -m venv venv
source venv/bin/activate  # или venv\Scripts\activate на Windows
pip install -r requirements.txt
uvicorn main:app --reload

Приложение запустится по адресу:
👉 http://127.0.0.1:8000/docs

🔑 Настройка Telegram-бота

Создайте бота через @BotFather

Напишите /newbot

Укажите имя и username (например, incident_alert_bot)

Скопируйте токен, который выдаст BotFather — он будет выглядеть так:

8478621884:AAFWVbZq0O37E02IrkKPICzopM7Y2Jx4SzU


Узнайте свой chat_id (ID чата, куда бот будет отправлять уведомления):

В Telegram откройте @userinfobot

Напишите /start

Бот ответит сообщением:

👤 Your ID: 123456789


Это и есть ваш chat_id.

Создайте файл .env в корне проекта:

BOT_TOKEN=8478621884:AAFWVbZq0O37E02IrkKPICzopM7Y2Jx4SzU
CHAT_ID=123456789


Проверьте, что бот работает:

Запустите сервер FastAPI.

Создайте инцидент через Swagger UI (/incidents/).

Убедитесь, что бот отправил уведомление в Telegram.

🧠 Эндпоинты
1. Создать инцидент
POST /incidents/
{
  "description": "Самокат не отвечает",
  "status": "open",
  "source": "operator"
}

2. Получить список инцидентов
GET /incidents?status=open

3. Обновить статус инцидента
PATCH /incidents/1
{
  "status": "resolved"
}

📘 Пример уведомления в Telegram
🆘 Новый инцидент!

ID: 1
Описание: Самокат не отвечает
Статус: open
Источник: operator
Создано: 2025-11-09 14:22:31


---
